...
def serve_connections():
    """ Listen to incoming request over TCP on binding PROXY_HOST:IN_PORT
        Opens a new thread to handle every client request"""
    client_sock_object = init_sock_object()
    while True:
        try:
            client_connection, client_address = client_sock_object.accept()
            query_data = client_connection.recv(1024)
            logging.info('Client {} connected'.format(client_address))
            logging.info('Query is {}'.format(query_data))
...
def init_sock_object():
    ...
    try:
        client_sock_object = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        client_sock_object.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        # client_sock_object.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
        client_sock_object.bind((PROXY_HOST, IN_PORT))
        client_sock_object.listen(1)
